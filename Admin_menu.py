...
class Admin:
    ...
    def admin_mainmenu(self, a, b):
        ...
        self.accounts.place(x=560, y=27)
        ...
        self.changeuser.place(x=735, y=27)

        # Debt button
        mi = PhotoImage(file="images/debt.png")
        mi = mi.subsample(a, b)
        self.debt = Button(self.mainframe, text="Debt", bd=5, font="roboto 11 bold", image=mi, compound=TOP, command=self.handle_debt)
        self.debt.image = mi
        self.debt.place(x=910, y=27)
        ...
        self.formframe2 = Frame(self.mainw, width=400, height=550, bg="#FFFFFF")
        self.formframe2.place(x=50, y=270)
    ...
    def get_debts(self):
        ...
        Button(self.formframe2, text="Remove", font="robot 11 bold",bg="#FFFFFF", bd=5, width=20, height=2,
        command=self.removedebt).place(x=160, y=110)
        Button(self.formframe2, text="Update", font="robot 11 bold",bg="#FFFFFF", bd=5, width=20, height=2,
        command=self.settledebt).place(x=160, y=210)
    ...
    def getsales(self):
        ...
        for i in range(0, len(saleslist)):
            ...
            if l:  # Check if l is not empty
                ...
                print(f"No product data found for product ID {saleslist[i][2]}")
        for i in saleslist:
            self.tree.insert('', 'end', values=(i))
    ...
    def searchinvoice(self):
        ...
        saleslist = self.cur.fetchall()
        for i in saleslist:
            if (str(i[1]) == str(self.searchvar.get())):
                self.cur.execute("select product_desc,product_price from products where product_id=?", (int(i[2]),))
                l = self.cur.fetchall()
                s = (str(i[4])).split('-')
                i = list(i)
                i[4] = s[2] + " - " + s[1] + " - " + s[0]
    ...
    def removedebt(self):
        ...
        cur = self.tree.item(cur)
        li = cur['values']
        if messagebox.askyesno('Alert!','Do you want to remove product from inventory?') == True and len(li) ==8:
            self.cur.execute("delete from debt where debt_id = ?;", (li[0],))
            self.base.commit()
            self.tree.delete(*self.tree.get_children())
            self.handle_debt()
